        ...
        <?php else: ?>
            <div class="admin-sub-container-2">
                ...
                <table class="admin-table">
                    <thead class="admin-table-header">
                        <tr class="admin-table-row">
                            <th class="admin-table-th">Question</th>
                            <th class="admin-table-th">Question Type</th>
                            <th class="admin-table-th">Rating</th>
                            <th class="admin-table-th">Status</th>
                            <th class="admin-table-th">Created On</th>
                        </tr>
                    </thead>
                    <tbody class="admin-table-body">
                        <?php foreach ($questions as $index => $question): ?>
                            <tr class="admin-table-row <?= $index % 2 === 0 ? 'admin-table-row--even' : 'admin-table-row--odd' ?>">
                                <td class="admin-table-td"><?= htmlspecialchars($question['question']) ?></td>
                                <td class="admin-table-td"><?= htmlspecialchars($question['questionType']) ?></td>
                                <td class="admin-table-td"><?= htmlspecialchars((string) $question['rating']) ?></td>
                                <td class="admin-table-td"><?= htmlspecialchars($question['status']) ?></td>
                                <td class="admin-table-td"><?= htmlspecialchars($question['createdOn']) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
