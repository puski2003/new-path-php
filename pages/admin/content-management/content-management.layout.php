...
<main class="admin-main-container">
    ...
    <section class="admin-main-content">
        ...
        <div class="admin-sub-container-1">
            <div class="admin-summary-card">
                <div class="admin-summary-card-content">
                    <p class="admin-summary-card-title">Total Reports Today</p>
            ...
            <div class="admin-summary-card">
                <div class="admin-summary-card-content">
                    <p class="admin-summary-card-title">Active Bans</p>
                    <p class="admin-summary-card-info"><?= $activeBans ?></p>
                </div>
            </div>
        </div>

        <div class="admin-sub-container-2">
            <form method="GET" class="content-management-filters">
                <div class="content-management-filters__dropdowns">
                    <input type="text" name="type" placeholder="Type" value="<?= htmlspecialchars($filters['type'] === 'all' ? '' : $filters['type']) ?>">
                    <input type="text" name="reason" placeholder="Reason" value="<?= htmlspecialchars($filters['reason'] === 'all' ? '' : $filters['reason']) ?>">
                    <input type="text" name="status" placeholder="Status" value="<?= htmlspecialchars($filters['status'] === 'all' ? '' : $filters['status']) ?>">
                    ...
                    <tbody class="admin-table-body">
                        <?php foreach ($reportedContent as $index => $item): ?>
                            <tr class="admin-table-row <?= $index % 2 === 0 ? 'admin-table-row--even' : 'admin-table-row--odd' ?>">
                                <td class="admin-table-td"><?= htmlspecialchars($item['contentPreview']) ?></td>
                                <td class="admin-table-td"><?= htmlspecialchars($item['authorName']) ?></td>
                                <td class="admin-table-td"><?= htmlspecialchars($item['type']) ?></td>
                                <td class="admin-table-td"><?= htmlspecialchars($item['reason']) ?></td>
                                <td class="admin-table-td"><?= htmlspecialchars($item['reportedByName']) ?></td>
                                <td class="admin-table-td"><?= htmlspecialchars($item['date']) ?></td>
                                <td class="admin-table-td"><?= htmlspecialchars(ucfirst($item['status'])) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <div class="admin-sub-container-1" style="align-items: stretch;">
            <div class="admin-data-card" style="flex: 1;">
                <h3>Trending Posts</h3>
                <p>Recovery milestone celebration</p>
                <p>Weekly support group meeting</p>
                <p>Healthy lifestyle tips</p>
            </div>
            <div class="admin-data-card" style="flex: 1;">
                <h3>Moderation Summary</h3>
                <p>Locked Threads: 5</p>
                <p>Pinned Posts: 3</p>
                <p>Active Warnings: 12</p>
            </div>
        </div>
    </section>
</main>
