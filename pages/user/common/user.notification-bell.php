...
(function () {
    ...
    function fetchNotifications() {
        loaded = true;
        fetch('/user/notifications?ajax=list')
    ...
    function markOneRead(id, el) {
        ...
        fetch('/user/notifications?ajax=mark_one_read', { method: 'POST', body: fd })
            .then(function () {
    ...
    markAllBtn.addEventListener('click', function () {
        fetch('/user/notifications?ajax=mark_read', { method: 'POST' })
            .then(function () {
                renderBadge(0);
    ...
    // Poll for unread count every 60 s (lightweight — just the badge)
    function pollBadge() {
        fetch('/user/notifications?ajax=list')
            .then(function (r) { return r.json(); })
            .then(function (data) {
    ...
    pollBadge(); // initial badge load on page load
}());
</script>
